...
<template>
  <div
    class="min-h-screen selection:bg-brand-accent selection:text-white"
    :class="{ dark: settingsStore.isDarkMode, light: !settingsStore.isDarkMode }"
  >
    <!-- Navigation -->
    <nav
      class="fixed top-4 left-4 z-50 flex items-center space-x-2 bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-2"
    >
      ...
      <div class="flex p-1.5 bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl">
        <button
          ...
          :style="{
            ...
            border: store.accentColor === color ? '2px solid white' : 'none',
          }"
        ></button>
      </div>
      <button
        @click="openSettings"
        class="p-3 bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl hover:bg-white/20 transition-all"
      >
        <Settings class="text-slate-300" :size="18" />
      </button>
      <button
      ...
      <header class="space-y-10">
        ...
        <div class="space-y-6">
          ...
          >
            ...
            <span class="text-slate-900 dark:text-white not-italic font-bold">Engineering</span> and
            <span class="text-slate-900 dark:text-white not-italic font-bold">Design</span>.
          </Motion>
        </div>
        ...
        >
          ...
          <button
            class="font-black uppercase tracking-widest text-xs flex items-center group"
        ...
        <div class="space-y-8">
          ...
          <div class="space-y-12">
            <div class="group space-y-4">
              <div
                class="w-12 h-0.5 bg-slate-200 dark:bg-slate-800 transition-all group-hover:w-20"
                :style="{ backgroundColor: store.accentColor }"
              ></div>
              <h4 class="text-2xl font-display font-bold">Systems Architecture</h4>
              <p class="text-sm text-slate-500 leading-relaxed">
                Designing scalable, resilient frontend infrastructures using modern reactive
                patterns and component-driven development.
              </p>
            </div>
            <div class="group space-y-4">
              <div
              ...
              <p class="text-sm text-slate-500 leading-relaxed">
                Aligning technical capabilities with user-centric design to deliver high-impact
        ...
        <div class="space-y-8">
          ...
          <div class="space-y-4">
            <div
              v-for="i in 3"
              :key="i"
              class="glass p-6 rounded-3xl flex items-center justify-between group hover:bg-white/5 transition-all cursor-pointer"
            >
              <div class="flex items-center space-x-4">
                <div
                  class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-lg"
                >
                  0{{ i }}
                </div>
      ...
      <section class="space-y-10">
        ...
        <div class="space-y-0 border-l border-slate-100 dark:border-slate-800 ml-4">
          <div v-for="exp in experience" :key="exp.year" class="relative pl-10 pb-12 group">
            <div
              class="absolute left-[-5px] top-1.5 w-2.5 h-2.5 rounded-full bg-slate-200 dark:bg-slate-800 group-hover:scale-150 transition-transform"
              :style="{ backgroundColor: store.accentColor }"
            ></div>
            <div class="space-y-1">
    ...
    <footer
      class="py-20 px-8 border-t border-slate-100 dark:border-slate-800 flex flex-col items-center space-y-8"
    >
